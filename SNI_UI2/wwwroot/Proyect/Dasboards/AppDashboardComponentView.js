...
class AppDashboardComponentView extends HTMLElement {
    ...
    async CreateDasboard() {
        ...
        const comment = WRender.Create({ className: "dashboard-comment" });
        ...
        data.comments.forEach(element => {
            comment.append(WRender.CreateStringNode(`<div class="case-dependencie cookieCard">               
                <p class="cookieHeading">${element.NickName}</p>
                <p class="cookieDescription">${element.Body} <a href="#">What for?</a></p>
                <button class="acceptButton">Ver.</button>
            </div>`));
        });

        data.task.forEach(element => {
            task.append(WRender.CreateStringNode(`<div class="case-task news">
                <figure>
                    <img src="http://www.churchillpark.nl/wp-content/themes/churchillpark/img/slide-2.png" alt="news-image"/>
                </figure>                
                <div class="category">${element.Descripcion}</div>                
                <h2>${element.Descripcion}</h2>                
                <div class="cta">
                    <a class="button">Lees meer</a>
                </div>
            </div>`));
        });
        ...
        component.append(dependencies, caseList, comment, task);
        return component;
    }
